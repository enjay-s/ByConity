...
namespace thrift
{
    namespace transport
    {
        ...
        TSaslTransport::TSaslTransport(std::shared_ptr<TTransport> transport)
            : TVirtualTransport(DefaultTConfiguration())
            , transport_(transport)
            , memBuf_(new TMemoryBuffer(32 * 1024, DefaultTConfiguration()))
            , sasl_(nullptr)
            , shouldWrap_(false)
            , isClient_(false)
        {
        ...
        TSaslTransport::TSaslTransport(std::shared_ptr<sasl::TSasl> saslClient, std::shared_ptr<TTransport> transport)
            : TVirtualTransport(DefaultTConfiguration())
            , transport_(transport)
            , memBuf_(new TMemoryBuffer(DefaultTConfiguration()))
            , sasl_(saslClient)
            , shouldWrap_(false)
            , isClient_(true)
        {
        ...
        void TSaslTransport::sendSaslMessage(const NegotiationStatus status, const uint8_t * payload, const uint32_t length, bool flush)
        {
